...
- **Agent-managed context.** Sessions are JSONL under `~/.wizard/sessions/`; auto-compaction keeps long runs inside the window; a live pressure signal and the mid-turn `compact` tool let the agent shed history on every surface (including headless/gateway); durable facts go to `memory` when the task changes. → [Agent-managed context](docs/usage.md#agent-managed-context)
