...
> **Read this first:** in 2.0.0 that line refuses. There is no signed release yet, and the installer will not put down something it cannot verify, so add `WIZARD_BUILD_FROM_SOURCE=1` (or use Nix, or a checkout). That, the other install flavors (local-stack preinstall, minimal, bring-your-own-model, macOS, Termux) and a first-run walkthrough are all in **[Getting started](docs/getting-started.md)**.
...
- **In 2.0.0 the installer does not download anything.** `wizard-release.pub` in this repository is still a placeholder rather than a real minisign public key, and `install.sh` refuses an unverifiable release rather than installing it: the check runs before the download, before the flavors, and before the source-build fallback further down, so every path that would download a release aborts in about a second with *"this install.sh carries no release signing key"*. `wizard update` refuses for the same reason. What works today is a source build, Nix, or a checkout. `WIZARD_BUILD_FROM_SOURCE=1` skips the check entirely and runs `cargo build --release`; Termux is the one platform where the plain one-liner still installs, because the installer forces that variable on there. The prebuilt download, and the fallback to a source build on a platform with no published asset, start working when a signed release exists. See [Getting started](docs/getting-started.md#install).
...
- [CHANGELOG.md](CHANGELOG.md): what changed in 2.0.0, what breaks coming from 1.x, and what to do about each break
