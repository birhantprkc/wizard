...
**Model fusion (`/fusion`).** Don't pick one model — fuse several. `/fusion` runs a panel of your configured providers as a debate (each answers, they critique each other over N rounds) and a synthesizer merges them into one tool-capable answer; the fusion of independent models tends to beat the best single one in the panel. It's just another provider under the hood, so tools, modes, and streaming are unchanged — the toggle swaps the active provider to the panel and back, and the status bar shows a loud `fusion: …` label because it spends several× the tokens. `/fusion config` picks the panel. See [docs/fusion.md](docs/fusion.md).
