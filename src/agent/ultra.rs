    ...
    pub fn build(cfg: &UltraConfig, user_dir: &Path) -> Result<Self> {
        ...
        for name in &cfg.lenses {
            ...
            lenses.push(SubagentConfig {
                ...
                max_steps: cfg.candidate_max_steps,
                tool_scope: None,
                ..found.clone()
            });
        }
...
/// additionally forces `dispatches_commands: false` and a fresh todo list into
/// the child context.
///
...
mod tests {
    ...
    fn ultra_overrides_a_lens_budget_and_tool_scope() {
        ...
        assert_eq!(engine.lenses[0].max_steps, 4, "ultra owns the budget");
        assert!(
            engine.lenses[0].tool_scope.is_none(),
            "a lens contributes a prompt, never a scope"
        );
        assert_eq!(engine.judge.max_steps, cfg.judge_max_steps);
