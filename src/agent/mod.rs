...
pub enum AgentEvent {
    ...
    /// later `Subagent*` event below to this run, so a surface can demux
    ...
    /// calls). Scoped to [`AgentEvent::SubagentRunStarted::run`].
    SubagentRunText { run: u64, text: String },
    /// A subagent started a tool call. Scoped to a run; the tool name is bare
    /// (not prefixed with the subagent's name — the pane supplies that).
...
impl Agent {
    ...
    /// kill a detached run. Cloned out (rather than borrowed through the
    /// agent) because the TUI parks the whole `Agent` elsewhere while a turn
    /// is in flight — which is exactly when you want to be able to kill a
    /// runaway subagent.
