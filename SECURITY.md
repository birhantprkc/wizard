...
Until the release keypair exists, `wizard-release.pub` holds a placeholder, and both the installer and `wizard update` refuse **every** release with a message naming that as the reason. That is deliberate: a self-updater that installs unverified binaries when no key is configured is the hole this section exists to close. Seeding it is one command, and the secret key never has to leave the machine that made it except to become a repository secret:
