...
- **As a client** it connects to external MCP servers declared in
  ...
  rebuild — the path for computer use, browser control, and databases. See
  [Self-extension](evolve.md#mcp-servers).
...
native tools (`read_file`, `write_file`, `edit_file`, `list_files`,
`search_files`, `execute`, `git_status`, `git_diff`, `memory`, `todo`,
`web_fetch`, `web_search`, `generate_image`, and the task/subagent controls).
